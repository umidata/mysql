<?php
// Copyright 2013-present Umidata.  All rights reserved.
// @author: rahulgulati (rahulgulati@fb.com)

class UmidataMySQLLintEngine extends ArcanistLintEngine {
  ...
  public function buildLinters() {
    ...
    // UmidataMySQLLinter enforces the following lint checks: max line length
    ...
    $linters[] = id(new UmidataMySQLLinter())->setPaths($text_paths);
    ...
    if (!$this->getCommitHookMode()) {
      // UmidataMySQLClangFormatLinter
      $clang_format_linter = id(new UmidataMySQLClangFormatLinter());
    ...
    $linters[] = id(new UmidataInnoDBTabLinter())
    ...
    $linters[] = id(new UmidataMySQLPrintfLinter())
        ->setPaths($all_cpp_paths);

    $linters[] = id(new UmidataMySQLAssertUsageLinter())
    ...
    if (strcmp(getenv("SKIP_HOWTOEVEN"), "1")) {
      ...
      $linters[] = id(new UmidataMySQLHowtoevenLinter())
          ->setPaths($myrocks_cpp_paths);
    }
    ...
    $linters[] = id(new UmidataMySQLTestResultLinter())
