      ...
      if (mysql->server_status & SERVER_SESSION_STATE_CHANGED)
      {
        ...
        while (total_len > 0)
        {
          ...
          case SESSION_TRACK_GTIDS:
            ...
            /* read (and ignore for now) the GTIDS encoding specification */
            (void) net_field_length(&pos);
            ...
            if(!(data->str= (char *)my_malloc(len + 1, MYF(MY_WME))))
            {
              set_mysql_error(mysql, CR_OUT_OF_MEMORY, unknown_sqlstate);
              return;
            }
...
void* STDCALL
mysql_get_ssl_session(MYSQL* mysql MY_ATTRIBUTE((unused))) {
...
my_bool mysql_get_ssl_session_reused(MYSQL* mysql MY_ATTRIBUTE((unused))) {
...
my_bool STDCALL
mysql_get_ssl_server_cerfificate_info(MYSQL *mysql MY_ATTRIBUTE((unused)),
                                      char* subject_buf MY_ATTRIBUTE((unused)),
                                      size_t subject_buflen MY_ATTRIBUTE((unused)),
...
const char* mysql_compression_lib_names[] = {"none", "zlib", "zstd", "zstd_stream", "lz4f_stream", NullS};
