...
Item::Item(THD *thd, Item *item):
  ...
  alias_was_set(item->alias_was_set),
...
/*
  Fix fields for the only two sub-classes Item_field and Item_ref. The only
  difference is that Item_field has member 'field' but Item_ref doesn't. This
  is handled by virtual function get_field() and should_fix_document_path().
  The actual work of fixing fields are done by virtual fix_fields_do().
 */
bool Item_ident::fix_fields(THD *thd, Item **reference)
{
  /*
...
Item::save_in_field_no_warnings(Field *field, bool no_conversions)
{
  ...
  {
    // Reset the document blob since the memory hasn't been
    // allocated yet and the pointer is not valid.
...
bool Item::is_blob_field() const
{
  ...
  /* Document type will be treated as blob */
  if (type == MYSQL_TYPE_BLOB || type == MYSQL_TYPE_GEOMETRY ||
      type == MYSQL_TYPE_DOCUMENT ||
  ...
  /*
    ...
    So we add and extra restriction, only in MySQL 5.7: if longer than this,
        ...
        if (res != (Item **)not_found_item)
        {
          if ((*res)->type() == Item::FIELD_ITEM)
          {
            ...
            */
            ...
            Item_field *item_field = *((Item_field**)res);
            if (item_field->field == NULL)
            {
...
Item::save_in_field(Field *field, bool no_conversions)
{
           ...
           field->result_type() == STRING_RESULT)
  {
    ...
    if (extra_args &&
      ...
      fbson::FbsonValue* pval = ((Field_document*)field)->get_fbson_value();
...
bool Item_ref::fix_fields_do(THD *thd, Item **reference)
{
  enum_parsing_place place= NO_MATTER;
  DBUG_ASSERT(fixed == 0);
